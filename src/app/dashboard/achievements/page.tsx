    ...
    <main className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">
          Earn badges by completing roadmap milestones · {earned.length}/
          {allAchievements.length} unlocked
        </p>
      </header>
      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 list-none p-0 m-0">
        {allAchievements.map((ach) => {
          ...
          return (
            <li key={ach.id}>
              <Card
                className={`relative h-full overflow-hidden transition-all ${isEarned ? "ring-1 ring-primary/50" : "opacity-50 grayscale"}`}
              >
                {isEarned && (
                  <div
                ...
                <CardContent className="p-6 text-center">
                  ...
                  <h2 className="text-lg font-bold">{ach.name}</h2>
