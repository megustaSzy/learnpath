...
async function UserDashboard() {
  ...
  const statCards = [
    {
      label: "Roadmaps Joined",
      value: stats.roadmapsJoined,
      icon: Map,
      color: "text-blue-400",
    },
    {
    ...
    <main className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track your learning progress across roadmaps
          </CardDescription>
        </CardHeader>
        <CardContent>
          {stats.roadmapProgress.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="mt-4 text-muted-foreground">
                You haven't joined any roadmaps yet.
              </p>
              <Link
            ...
            <ul className="space-y-4 list-none p-0 m-0">
              {stats.roadmapProgress.map((rp) => (
                <li key={rp.id}>
                  <Link
                    href={`/dashboard/progress/${rp.slug}`}
                    className="block"
                  >
                    <div className="flex items-center gap-4 rounded-lg border p-4 hover:bg-accent/50 transition-colors">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {rp.category} · {rp.completedTopics}/{rp.totalTopics}{" "}
                          topics
                        </p>
                      </div>
...
async function SuperAdminDashboard() {
  ...
  return (
    <main className="space-y-6">
      ...
      <section
        aria-label="Super Admin Statistics"
        className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"
      >
        {statCards.map((stat) => (
          <Card key={stat.label}>
